...
class Spp_patch2(nn.Module): 
  def __init__(self, m:nn.Module,patch,pixel):
    ...
    self.pool_add_1 = nn.AvgPool2d(kernel_size=(1,768),stride=(1,768),padding=0)  
    self.pool_add_2 = nn.AvgPool2d(kernel_size=(196,1),stride=(196,1),padding=0)  
    
    self.SE_add_1 = nn.Sequential(OrderedDict([
                      ('SE1_fc1',  nn.Linear(196,16)),
                      ('SE1_ReLU',nn.ReLU()),
                      ('SE1_fc2',nn.Linear(16,196)),
                      ('SE1_Sigmoid',nn.Sigmoid())
                          ]))
    self.SE_add_2 = nn.Sequential(OrderedDict([
  ...
  def forward(self,input):
    ...
    # # patch
    out_pool_1 = self.pool_add_1(out_layer2_reshape)      
    out_pool_11 = out_pool_1.squeeze(1).squeeze(2)    
    out_SE1 = self.SE_add_1(out_pool_11)
    out_SE11 = out_SE1.unsqueeze(-1)  
    out_SE12 = out_SE11*self.patch
    
    # # pixel
    out_pool_2 = self.pool_add_2(out_layer2_reshape)   
    out_pool_21 = out_pool_2.squeeze(1).squeeze(1)
    out_SE2 = self.SE_add_2(out_pool_21)
